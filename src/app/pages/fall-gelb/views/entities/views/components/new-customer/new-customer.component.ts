...
export class NewCustomerComponent implements OnInit {
  customerForm: FormGroup = this.fb.group({
    ...
    taxpayer: this.fb.control(
      this.data.dialogData
        ? this.data.dialogData[0].special_tax_payer === true
          ? 1
          : 2
        : '',
      [Validators.required]
    ...
    biller: this.fb.control(
      this.data.dialogData ? this.data.dialogData[0].biller_id : '',
      [Validators.required]
    ),
  });
  ...
  getBillers() {
    this.customersService.getBillers().subscribe((resp) => {
    ...
    if (this.customerForm.valid) {
      if (this.data.title === 'Crear Cliente') {
        const customer = {
          ...
          special_tax_payer:
            this.customerForm.get('taxpayer')?.value === 1 ? true : false,
        ...
        this.customersService
          .editCustomer(customerEdit, this.data.dialogData[0].id)
          .subscribe(
            (data) => {
