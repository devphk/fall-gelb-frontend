...
import { FormCustomerComponent } from '../components';
...
export class CustomersComponent implements OnInit {
  ...
  tableColumnsTags: string[] = ['id', 'name', 'phone', 'address', 'biller'];
  tableData: any[] = [];
  itemsSelected: any[] = [];
  billers = [];
  ...
  getCustomers() {
    this.customersService.getBillers().subscribe((billers) => {
      ...
      .subscribe((response) => {
        if (response) {
          this.customersService
            .deleteCustomer(this.itemsSelected[0].id)
            .subscribe(
              (data) => {
